...
import { reviewTimeOffRequestForManager } from "@/lib/data/manager";
...
export async function POST(
  ...
  if (user.accountType !== "MANAGER" && user.accountType !== "ADMIN") {
    return corsJson({ error: "Only managers can review time off requests" }, { status: 403 });
  }
  ...
  try {
    await reviewTimeOffRequestForManager(user, requestId, body.decision);
