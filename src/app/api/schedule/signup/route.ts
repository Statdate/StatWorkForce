...
import { signUpForShiftAsUser } from "@/lib/data/worker";
...
export async function POST(request: Request) {
  ...
  try {
    await signUpForShiftAsUser(user, shiftId);
    return corsJson({ ok: true });
  } catch (error) {
    return corsJson({ error: error instanceof Error ? error.message : "Sign-up failed" }, { status: 400 });
  }
}
